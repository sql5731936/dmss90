...
Class Master extends DBConnection {
	...
	function save_top(){
		...
		$check = $this->conn->query("SELECT * FROM `top` where `name` = '{$name}' and id >= 0 ".($id > 0 ? " and id != '{$id}' " : '')." ")->num_rows;
		if($check > 0){
			$resp['status'] = 'failed';
			$resp['msg'] = 'Dorm Name already exists.';
			return json_encode($resp);
		...
		if(empty($id)){
			$sql = "INSERT INTO `top` set {$data} ";
		}
		
		else{
			$sql = "UPDATE `top` set {$data} where id = '{$id}' ";
		}
		////////////////kimi//                 else{
	...
	function delete_top(){
		extract($_POST);
		$del = $this->conn->query("DELETE from `top`  where id = '{$id}'");
		if($del){
			$resp['status'] = 'success';
			$this->settings->set_flashdata('success'," top successfully deleted.");
		}else{
			$resp['status'] = 'failed';
			$resp['error'] = $this->conn->error;
		}
	...
	function save_kimi(){
		...
		$check = $this->conn->query("SELECT * FROM `kimi` where `name` = '{$name}' and id >= 0 ".($id > 0 ? " and id != '{$id}' " : '')." ")->num_rows;
		if($check > 0){
			$resp['status'] = 'failed';
			$resp['msg'] = 'Dorm Name already exists.';
			return json_encode($resp);
		...
		if(empty($id)){
			...
			$sql = "INSERT INTO `kimi`  (name,age,image)value('$name','$age','$image') ";
		}
			...
			$imageName = $_FILES['image']['name'];
			$image = addslashes(file_get_contents($_FILES['image']['tmp_name']));
			$sql =  "UPDATE kimi SET name='$name', age='$age',image='$image' where id = '{$id}' ";
		}
		////////////////kimi//                 else{
			//////$sql = "DELETE from `top`  where id = '{$id}'";
	...
	function delete_kimi(){
		extract($_POST);
		$del = $this->conn->query("DELETE from `kimi`  where id = '{$id}'");
		if($del){
			$resp['status'] = 'success';
			$this->settings->set_flashdata('success'," kimi successfully deleted.");
		}else{
			$resp['status'] = 'failed';
			$resp['error'] = $this->conn->error;
		}
...
switch ($action) {
	...
	case 'save_top':
		echo $Master->save_top();
	break;
	case 'delete_top':
		echo $Master->delete_top();
	break;



	case 'save_kimi':
		echo $Master->save_kimi();
	break;
	case 'delete_kimi':
		echo $Master->delete_kimi();
	break;
